...
//forward decleration
class ofxTCPClientNullTerminated;

class ofxTCPServerNullTerminated : public ofThread{

	public:

		ofxTCPServerNullTerminated();
		~ofxTCPServerNullTerminated();

		// ofxTCPServerNullTerminated can't be copied to avoid problems with destruction
		ofxTCPServerNullTerminated(const ofxTCPServerNullTerminated & mom) = delete;
		ofxTCPServerNullTerminated & operator=(const ofxTCPServerNullTerminated & mom) = delete;
	...
	private:
		ofxTCPClientNullTerminated & getClient(int clientID);
		bool isClientSetup(int clientID);

		void threadedFunction();

		ofxTCPManagerNullTerminated			TCPServer;
		std::map<int,std::shared_ptr<ofxTCPClientNullTerminated> >	TCPConnections;
