...
//--------------------------
ofxTCPClientNullTerminated::ofxTCPClientNullTerminated(){
...
//--------------------------
void ofxTCPClientNullTerminated::setVerbose(bool _verbose){
    ofLogWarning("ofxTCPClientNullTerminated") << "setVerbose(): is deprecated, replaced by ofLogWarning and ofLogError";
}

//--------------------------
bool ofxTCPClientNullTerminated::setup(string ip, int _port, bool blocking){

	if( !TCPClient.Create() ){
		ofLogError("ofxTCPClientNullTerminated") << "setup(): couldn't create client";
		return false;
	}else if( !TCPClient.Connect((char *)ip.c_str(), _port) ){
		ofLogError("ofxTCPClientNullTerminated") << "setup(): couldn't connect to " << ip << " " << _port;
...
//--------------------------
bool ofxTCPClientNullTerminated::setupConnectionIdx(int _index, bool blocking){
	index = _index;
...
//--------------------------
bool ofxTCPClientNullTerminated::close(){
	if( connected ){
		if( !TCPClient.Close() ){
			ofLogError("ofxTCPClientNullTerminated") << "close(): couldn't close client";
			return false;
		}else{
            ofLogVerbose("ofxTCPClientNullTerminated") << "closing client";
			connected = false;
			return true;
		}
	}else{
...
//--------------------------
void ofxTCPClientNullTerminated::setMessageDelimiter(string delim){
	if(delim != ""){
...
//--------------------------
bool ofxTCPClientNullTerminated::send(string message){
	...
	// other applications
	if(!connected){
		ofLogWarning("ofxTCPClientNullTerminated") << "send(): not connected, call setup() first";
		return false;
	}
	...
	if( isClosingCondition(ret, errorCode) ){
		ofLogWarning("ofxTCPClientNullTerminated") << "send(): client disconnected";
		close();
		return false;
	}else if(ret<0){
		ofLogError("ofxTCPClientNullTerminated") << "send(): sending failed";
		return false;
...
//--------------------------
bool ofxTCPClientNullTerminated::sendRawMsg(const char * msg, int size){
	...
	// other applications
	if(!connected){
		ofLogWarning("ofxTCPClientNullTerminated") << "sendRawMsg(): not connected, call setup() first";
		return false;
	}
    ...
    if(ret<0) errorCode = ofxNetworkCheckError();
	if( isClosingCondition(ret, errorCode) ){
		ofLogWarning("ofxTCPClientNullTerminated") << "sendRawMsg(): client disconnected";
		close();
		return false;
	}else if(ret<0){
		ofLogError("ofxTCPClientNullTerminated") << "sendRawMsg(): sending failed";
		return false;
	}else if(ret<size){
		// in case of partial send, store the
		// part that hasn't been sent and send
...
//--------------------------
bool ofxTCPClientNullTerminated::sendRaw(string message){
	if( message.length() == 0) return false;
    int ret = TCPClient.SendAll(message.c_str(), message.length());
    int errorCode = 0;
    if(ret<0) errorCode = ofxNetworkCheckError();
	if( isClosingCondition(ret, errorCode) ){
		ofLogError("ofxTCPClientNullTerminated") << "sendRawBytes(): sending failed";
		close();
		return false;
	}else{
		return ret > 0;
...
//--------------------------
bool ofxTCPClientNullTerminated::sendRawBytes(const char* rawBytes, const int numBytes){
...
//--------------------------
int ofxTCPClientNullTerminated::getNumReceivedBytes(){
	return messageSize;
}
...
//--------------------------
bool ofxTCPClientNullTerminated::isClosingCondition(int messageSize, int errorCode){
...
//--------------------------
string ofxTCPClientNullTerminated::receive(){
	str    = "";
	int length=0;
...
//--------------------------
int ofxTCPClientNullTerminated::receiveRawMsg(char * receiveBuffer, int numBytes){
	int length=-2;
	//only get data from the buffer if we don't have already some complete message
...
//--------------------------
int ofxTCPClientNullTerminated::receiveRawBytes(char * receiveBuffer, int numBytes){
...
//--------------------------
int ofxTCPClientNullTerminated::peekReceiveRawBytes(char * receiveBuffer, int numBytes){
...
//--------------------------
string ofxTCPClientNullTerminated::receiveRaw(){
...
//--------------------------
bool ofxTCPClientNullTerminated::isConnected(){
	if (connected) {
...
//--------------------------
int ofxTCPClientNullTerminated::getPort(){
	return port;
}

//--------------------------
string ofxTCPClientNullTerminated::getIP(){
	return ipAddr;
}
